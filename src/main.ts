  ...
  <main class="console-shell">
    ...
    <section class="deck">
      ...
      <section class="main-stack">
        <div class="workspace-tabs"><button id="app-launcher" class="app-launcher" aria-expanded="false">APPS +</button><button class="app-tab active" data-workspace="console">CONSOLE</button><button class="app-tab" data-workspace="files">FILES</button><button class="app-tab" data-workspace="network">IP INTEL</button><button class="app-tab" data-workspace="radios">RADIOS</button><div id="app-menu" class="app-menu" hidden><p>OPEN APPLICATION</p><button data-workspace="console">COMMAND CONSOLE <span>F1</span></button><button data-workspace="files">FILESYSTEM <span>F2</span></button><button data-workspace="network">IP INTELLIGENCE <span>F3</span></button><button data-workspace="radios">RADIO MONITOR <span>F4</span></button></div></div>
...
window.addEventListener("keydown", (event) => {
  ...
  if (event.key === "F2" && document.activeElement?.tagName !== "INPUT" && document.activeElement?.tagName !== "TEXTAREA") { event.preventDefault(); showWorkspace("files"); document.querySelector<HTMLInputElement>("#file-path")!.focus(); }
...
const showWorkspace = (name: string) => {
  ...
  document.querySelectorAll<HTMLButtonElement>(".app-tab").forEach((button) => button.classList.toggle("active", button.dataset.workspace === name));
  appMenu.hidden = true; appLauncher.setAttribute("aria-expanded", "false");
...
const appLauncher = document.querySelector<HTMLButtonElement>("#app-launcher")!;
const appMenu = document.querySelector<HTMLDivElement>("#app-menu")!;
appLauncher.addEventListener("click", () => { appMenu.hidden = !appMenu.hidden; appLauncher.setAttribute("aria-expanded", String(!appMenu.hidden)); });
